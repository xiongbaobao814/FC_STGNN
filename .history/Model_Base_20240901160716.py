...
class Dot_Graph_Construction_weights(nn.Module):
    ...
    def forward(self, node_features):
        ...
        node_features = self.mapping(node_features)     # [batch_size, num_nodes, input_dim]
...
def iDot_Graph_Construction(node_features):
    ...
    ## output size is (bs, N, N)#-
    """#+
    Construct a graph adjacency matrix using inner product of node features.#+
#+
    Parameters:#+
    node_features (torch.Tensor): A tensor representing node features. The size is (bs, N, dimension).#+
#+
    Returns:#+
    torch.Tensor: A tensor representing the graph adjacency matrix. The size is (bs, N, N).#+
    """#+
    bs, N, dimen = node_features.size()
...
class MPNN_mk_v2(nn.Module):
    def __init__(self, input_dimension, output_dinmension, k):
        ...
        # two choices 'cat' (concatnate) or 'sum' (sum up) 
        self.way_multi_field = 'sum'    # 决定多个感受野的结果是拼接还是相加
        self.k = k
        theta = []
        for kk in range(self.k):
            theta.append(nn.Linear(input_dimension, output_dinmension))
...
def Conv_GraphST(input, time_window_size, stride):
    ...
    y_ = torch.transpose(y_, 1,-1)  # [bs, num_windows, num_sensors, time_window_size, feature_dim]
    return y_
