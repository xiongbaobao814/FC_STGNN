...
# 训练模型：通过循环进行多个epoch的训练，每个epoch中训练一批数据，并在每个show_interval间隔时进行交叉验证。
    def Train_model(self):
        ...
        for i in range(epoch):
            ...
            print(i)
        print(epoch)
        print(real_, type(real_[0]))
        print('test_accu_:', test_accu_)
        print('prediction_:', prediction_[0].numel)
        print('real_:', real_)
        # test_accu_ = np.array(test_accu_)
        # prediction_ = np.array(prediction_)
        # real_ = np.array(real_)
        # print(type(real_), type(real_[0]))
        # combined_array = np.column_stack((test_accu_, prediction_, real_))
        # np.save('./experiment/test.npy', combined_array)
        # np.save('./experiment/{}.npy'.format(self.args.save_name),[test_accu_, prediction_, real_])
        
        # # 打开文件以写入（如果文件不存在则创建）  
        # with open('./experiment/output.txt', 'w') as file:  
        #     # 遍历列表的索引  
        #     for i in range(len(test_accu_)):  
        #         # 将三个列表的对应元素写入文件，元素之间用空格（或任何你想要的分隔符）分隔  
        #         # 注意：这里使用了end=''来防止print函数自动添加换行符  
        #         file.write(f"{test_accu_[i]} {prediction_[i]} {real_[i]}\n")  
