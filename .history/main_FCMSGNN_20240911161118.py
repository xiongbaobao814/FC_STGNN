...
from dealdata_FCMSGNN.data_loader_ETT import data_generator


class Train():
    def __init__(self, args):
        self.train, self.valid, self.test = data_generator('/home/shilin/code/all_datasets/ETT-small/', args=args)
        self.args = args
...
# 计算整个训练集的累计损失值
    def Train_batch(self):
        ...
        for data, label in self.train:
            data = data.cuda() if torch.cuda.is_available() else data       # [100, 2, 9, 64]
            label = label.cuda() if torch.cuda.is_available() else label    # [100]
            self.optim.zero_grad()
            prediction = self.net(data)                                     # [100, 6]
...
# 训练模型：通过循环进行多个epoch的训练，每个epoch中训练一批数据，并在每个show_interval间隔时进行交叉验证
    def Train_model(self):
        ...
        print('real_:', real_)
        # test_accu_ = np.array(test_accu_)
        # prediction_ = np.array(prediction_)
        # real_ = np.array(real_)
        # combined_array = np.column_stack((test_accu_, prediction_, real_))
        # np.save('./results/HAR_results.npy', combined_array)
