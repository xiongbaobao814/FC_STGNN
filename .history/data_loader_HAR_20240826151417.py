...
def data_generator(data_path, args):
    ...
    train_loader = Dataset.DataLoader(dataset=train_dataset, 
                                        batch_size=args.batch_size,
                                        shuffle=True, 
                                        drop_last=args.drop_last, 
                                        num_workers=0)
    valid_loader = Dataset.DataLoader(dataset=valid_dataset, 
                                        batch_size=args.batch_size,
                                        shuffle=False, 
                                        drop_last=args.drop_last, 
                                        num_workers=0)
    test_loader = DataLoader.DataLoader(dataset=test_dataset, 
                                        batch_size=args.batch_size,
                                        shuffle=False, 
                                        drop_last=False, 
                                        num_workers=0)
