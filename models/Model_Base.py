...
class Feature_extractor_1DCNN_HAR_SSC(nn.Module):
    ...
    def forward(self, x_in):    # [1800, 64, 1]
        # print('input size is {}'.format(x_in.size()))
        x = torch.transpose(x_in, -1,-2)
        x = self.conv_block1(x)     # [1800, 48, 33]
        x = self.conv_block2(x)     # [1800, 96, 17]
...
class MPNN_mk_v2(nn.Module):
    ...
    def forward(self, X, A):    
        ...
        # 批量归一化
        GCN_output_ = torch.transpose(GCN_output_, -1, -2)  # [100, 16, 18]
...
class GraphConvpoolMPNN_block_v6(nn.Module):
    ...
    def forward(self, input):   # [100, 2, 9, 32]
        ...
        input_con_ = torch.transpose(input_con, 2, 3)  # [100, 1, 2, 9, 32]
        ...
        A_input = A_input*self.pre_relation                 # [100, 18, 18]
        input_con_ = torch.transpose(input_con_, -1, -2)    # [100, 32, 18]
        input_con_ = self.BN(input_con_)
        
        # 图卷积操作
        input_con_ = torch.transpose(input_con_, -1, -2)    # [100, 18, 32]
        X_output = self.MPNN(input_con_, A_input)           # [100, 18, 16]
